"""
Evaluation entrypoint for M4 (Global encoder + Transformer decoder).

Loads a saved m4 checkpoint, decodes captions over the val set, saves the
predictions, and computes BLEU / METEOR / CIDEr / ROUGE.

Mirrors evaluation/m1_eval.py and m3_eval.py so the team gets the same eval
format across models. Architecture is rebuilt from the saved checkpoint
config so we don't have to keep configs/m4_config.py in lockstep with the
trained model.

Usage:
    uv run python -m evaluation.m4_eval [decode_strategy] [beam_size] [length_penalty]
...
Examples:
    uv run python -m evaluation.m4_eval               # greedy
    uv run python -m evaluation.m4_eval beam          # beam, size 3, lp 0.7
    uv run python -m evaluation.m4_eval beam 5 0.6    # beam, size 5, lp 0.6
...
def main():
    ...
    d_model            = ckpt_config.get("d_model",            cfg.d_model)
    nhead              = ckpt_config.get("nhead",              cfg.nhead)
    num_decoder_layers = ckpt_config.get("num_decoder_layers", cfg.num_decoder_layers)
    dim_feedforward    = ckpt_config.get("dim_feedforward",    cfg.dim_feedforward)
    ...
    print(f"Checkpoint d_model: {d_model}, nhead: {nhead}, num_decoder_layers: {num_decoder_layers}")
