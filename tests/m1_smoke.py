"""
Quick smoke test for M1 (Global encoder + LSTM decoder, no attention).

Runs without any real data; random images and random caption ids. We check:
...
def make_model(embedding_type, device):
    """
    Builds an M1 with a random GloVe-shaped matrix when needed so the
