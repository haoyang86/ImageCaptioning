...
\end{center}
   \caption{Overview of the unified image captioning framework
    with interchangeable encoder and decoder components.}
\label{fig:framework}
\end{figure}
...
\teamtodo{Per the rubric, the report must mention any code
repositories or other sources used and what changes were made. 
I referenced some of my code from Assignment 2 and 3, 
but did not reuse anything verbatim. \href{https://github.com/haoyang86/ImageCaptioning/tree/main}{Our GitHub Repo}}
...
\paragraph{Loss function.} All models were trained to minimize the
cross-entropy loss, which calculates the per-token probability error
of the generated caption against the ground truth.

\paragraph{Optimizer.} The AdamW~\cite{loshchilov2019decoupled}
...
\paragraph{Transfer Learning.} Due to smaller dataset (Flickr8k),
...
\paragraph{Training results.} Figure~\ref{fig:loss} illustrates the
...
\end{center}
   \caption{Training convergence comparison between M1 (left) and
   M3 (right).}
\label{fig:loss}
\end{figure}
...
\end{tabular}
\end{center}
\caption{Quantitative architecture comparison (best hyperparameter
model). B-$n$: BLEU-$n$, MET.: METEOR, R-L: ROUGE-L.}
\label{tab:arch}
\end{table}
...
\end{tabular}
\end{center}
\caption{Metrics results for baseline M1 (Global$+$LSTM).}
\label{tab:emb-m1}
\end{table}
...
\end{tabular}
\end{center}
\caption{Metrics results for M2 (Spatial$+$LSTM Attention).}
\label{tab:emb-m2}
\end{table}
...
\end{tabular}
\end{center}
\caption{Metrics results for M3 (Spatial$+$Transformer).}
\label{tab:emb-m3}
\end{table}
...
\end{center}
   \caption{Qualitative comparison of generated captions across three difficulty levels (easy, medium, and challenging).}
\label{fig:qual}
\end{figure*}

\section*{Work Division}

\teamtodo{Paragraph description not yet written. The rubric asks for
both a short paragraph and the table below.}
...
Oscar A.~Martinez & 
  ...
  Creation of Optuna scripts for hyperparameter tuning (M1 and M4; unused) \\
\bottomrule
\end{tabular}
\end{center}
