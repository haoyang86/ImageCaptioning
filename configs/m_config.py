...
if embedding_type == "random":
    ...
    embedding_name = "glove_frozen"

elif embedding_type == "pretrained_finetune":
    embedding_name = "glove_finetune"

else:
    raise ValueError(f"Unsupported embedding_type: {embedding_type}")


lr_name = str(learning_rate).replace(".", "p").replace("-", "m")
dropout_name = str(dropout).replace(".", "p")

run_name = (
    f"layers{num_lstm_layers}_"
    f"att{attention_dim}_"
    f"hid{hidden_dim}_"
    f"lr{lr_name}_"
    f"drop{dropout_name}_"
    f"ep{num_epochs}"
)

experiment_name = f"{model_type}_{embedding_name}_{run_name}"
