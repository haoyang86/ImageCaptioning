...
def train_one_epoch(model, loader, criterion, optimizer, device, vocab_size, epoch):
    model.train()
    total_loss = 0.0
...
def save_checkpoint(path, model, optimizer, epoch, train_loss, val_loss, vocab):
    checkpoint = {
        "epoch": epoch,
...
def main():
    ...
    output_dir = project_root / cfg.checkpoint_dir
    output_dir.mkdir(parents=True, exist_ok=True)

    best_ckpt_path = output_dir / cfg.best_checkpoint_name
    last_ckpt_path = output_dir / cfg.last_checkpoint_name
    ...
    if should_use_pretrained_embedding(cfg.embedding_type):
        glove_path = project_root / cfg.embedding_path
